(function ($) {
    ...
    $.fn.cf7sgWarning = function (msg) {
        ...
        var $warning = $(
            '<span class="cf7sg-validation-warning">' + msg + '<span class="confirm-button">ok</span></span>'
        );
        $this.after($warning);
        ...
        if ($cf7Form_table.length) {
            $('.container.cf7-sg-table', $cf7Form_table).each(function () {
                ...
                if ($table[0].hasAttribute('id')) {
                    ...
                    var $tracker = $('<input class="cf7sg-tracker-field" value="1" type="hidden">').attr(
                        'name',
                        $table.attr('id')
                    );
                ...
                if ($footer.length > 0) {
                    ...
                    $table.after($footer);
                    $footer.after(
                        '<div class="cf7-sg-table-button"><a href="javascript:void(0);" class="ui-button">' +
                            label +
                            '</a></div>'
                    );
                } else {
            ...
            $cf7Form_table.click('.container', function (event) {
                ...
                if ($button.is('div.cf7-sg-table-button a')) {
                    //----------add a row
                    ...
                    $table.cf7sgCloneRow();
                } else if ($button.is('.cf7-sg-table .row-control .dashicons')) {
                    //---------- delete the row, delete button only on last row
        ...
        if ($cf7Form_tabs.length) {
            ...
            $('.cf7-sg-tabs', $cf7Form_tabs).each(function () {
                ...
                if (1 == $list.children('li').length) {
                    $list.after(
                        '<ul class="cf7sg-add-tab ui-tabs-nav"><li class="ui-state-default ui-corner-top"><a class="cf7sg-add-tab ui-tabs-anchor"><span class="cf7sg-add-tab dashicons dashicons-plus"></span></a></li></ul>'
                    );
                    //clone the tab
            ...
            $cf7Form_tabs.click('ul.ui-tabs-nav li', function (event) {
                ...
                } else if ($target.is('.cf7sg-add-tab')) {
                    //------------------- add tab.
                    //add a new tab
        ...
        if (cf7Form_accordion.length > 0) {
            ...
            cf7Form_accordion.filter('div.has-toggles form.wpcf7-form').each(function () {
                ...
                toggled_accordion.each(function () {
                    ...
                    if (0 == $cf72post.length) {
                        //disable the input fields in toggled sections.
                        if (!toggled) {
                ...
                form.click(toggled_accordion, function (event) {
                    ...
                    var $target = $(event.target);
                    if (
                        $target.is('span.cf7sg-title.toggled') ||
                        $target.is('.toggle-on') ||
                        $target.is('.toggle-off')
                    ) {
                    ...
                    if ($header.hasClass('ui-state-active')) {
                        ...
                        $('.row.ui-accordion-content :input', $header.parent())
                            .not('.cf7-sg-cloned-table-row :input')
                            .prop('disabled', false);
                        if (trackToggle) {
            ...
            if (nonceID.length > 0) {
                $form.on(nonceID, function (event) {
                    $('.cf7sg-collapsible.with-toggle', $(this)).each(function () {
                        ...
                        var id = $this.attr('id');
                        if (
                            'undefined' == typeof cf7sg.toggles_status ||
                            'undefined' == typeof cf7sg.toggles_status[id]
                        ) {
  ...
  */
    ...
    $.fn.cf7sgCloneRow = function (initSelect) {
        ...
        $(':input', $row).each(function () {
            ...
            $input
                .closest('span.wpcf7-form-control-wrap')
                .removeClass(name)
                .addClass(name + '_row-' + rowIdx);
