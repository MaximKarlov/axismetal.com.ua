(function () {
	...
	(this.$ipsp = function (t) {
		...
		(this.$ipsp.get = function (t) {
			if (!i[t]) throw Error('module is undefined');
			return new i[t]();
		}),
		(this.$ipsp.add = function (t, e) {
			if (i[t]) throw Error('module already added');
			i[t] = e;
		}),
	...
	(function () {
		this.addListener = function (t, i, e) {
			...
			t.addEventListener
				...
				: t.attachEvent && t.attachEvent('on' + i, e);
		};
	})(),
	(function () {
	...
	function () {
		this.prepareFormData = function (t, i, e, n) {
			...
			(s.action = t), (s.target = e || '_self'), (s.method = n || 'POST');
			for (var r in i)
				i.hasOwnProperty(r) &&
	...
	(function (t) {
		function i(t, i) {
			var e = !1,
				...
				c = r ? '' : 'on',
				u = function (n) {
					('readystatechange' === n.type && 'complete' !== o.readyState) ||
						(('load' === n.type ? t : o)[h](c + n.type, u, !1),
			...
			if ('complete' === o.readyState) i.call(t, 'lazy');
			else {
				if (!r && s.doScroll) {
					try {
						n = !t.frameElement;
	...
	function () {
		this.Module = Class.extend({
			...
			fireEvent: function (t, i) {
				...
				if (t)
					return document.createEventObject
						? ((e = document.createEventObject()), t.fireEvent('on' + i, e))
			...
			onDomReady: function (t) {
				return domReady(this.proxy(t)), this;
			},
	...
	function () {
		...
		(this.Connector = Module.extend({
			...
			getTarget: function () {
				return this.target || { postMessage: function () {} };
			},
			route: function (t) {
				...
				return (
					this.getTarget() === t.source &&
					!!i.action &&
					('origin' == i.action && this.setOrigin(i.data.origin),
			...
			removeAction: function (t) {
				return pubsub.unsubscribe(t), this;
			},
			publish: function (t, i) {
				return pubsub.publish([this.name, t].join('/'), i);
			},
			send: function (t, i) {
				this.getTarget().postMessage(
					JSON.stringify({ action: t, data: i }),
					this.getOrigin()
				);
			},
		})),
	...
	function (t) {
		var i = t.host,
			...
			r = function (t, i) {
				var e;
				if ('redirect' === t.action) return void this.loadUrl(t.url);
				'processing' !== t.send_data.order_status &&
					(this.unbind('ready').action('ready', function () {
						this.show();
					}),
					t.send_data &&
						t.url &&
						((e = prepareFormData(t.url, t.send_data)),
			...
			a = Module.extend({
				...
				config: function (t) {
					return this.extend(this.options, t), this;
				},
				setModal: function (t) {
				...
				setCheckoutBg: function (t) {
					this.onDomReady(function () {
						this.addCss(this.iframe, { background: t });
					});
				},
				setCheckoutWidth: function (t) {
					return (
						(t >= 0 || 'auto' == t) &&
							this.onDomReady(function () {
								this.addCss(this.iframe, { width: '' }),
									this.addAttr(this.iframe, { width: t });
							}),
						this
					);
				},
				width: function (t) {
					return this.setCheckoutWidth(t), this;
				},
				setCheckoutHeight: function (t) {
					(t >= 0 || 'auto' == t) &&
						this.onDomReady(function () {
							this.addCss(this.iframe, { height: '' }),
								this.addAttr(this.iframe, { height: t });
						});
				},
				height: function (t) {
					return this.setCheckoutHeight(t), this;
				},
				setCheckoutOrigin: function (t) {
					if (!t) throw Error('url param is required');
					var i = document.createElement('a');
					return (
						(i.href = t),
						(this.options.checkout_url =
							i.origin || [i.protocol, i.host].join('//')),
						this
					);
				},
						...
						for (i = 0; i < e.length; i++)
							this.addEvent(
								...
								this.proxy(function (t) {
									t.preventDefault(),
										this.loadUrl(
											t.target.getAttribute(this.options.checkout_attr)
										);
								})
							);
					});
				},
				legacyUrl: function (t) {
					var i = String(t).match(/(?:fondy.+?)(token|button)=(.+)&?/),
						e = 'api.fondy.eu';
					return null === i
						? t
						: (String(t).match(/dev/) && (e = 'api.dev.fondy.eu'),
						  (t = ['https://', e, '/api/checkout?', i[1], '=', i[2]].join(
								''
						  )));
				},
				loadUrl: function (t, i) {
					if (!t) throw Error('checkout url is not defined');
					(t = this.legacyUrl(t)),
				...
				show: function (t) {
					return (
						!0 === this.options.ismodal &&
							(this.addCss(this.find('body'), { overflow: 'hidden' }),
							this.addCss(this.find('html'), { overflow: 'hidden' })),
						this.addCss(this.iframe, { display: 'block' }),
						this.iframe.focus(),
				...
				hide: function (t) {
					return (
						!0 === this.options.ismodal &&
							(this.addCss(this.find('body'), { overflow: '' }),
							this.addCss(this.find('html'), { overflow: '' })),
				...
				send: function (t, i) {
					return this.connector.send(t, i), this;
				},
				publish: function (t, i) {
				...
				action: function (t, i) {
					...
					return (
						isArray(this.actions[t]) || (this.actions[t] = []),
						this.actions[t].push(e),
						e
					);
				},
				removeAction: function (t) {
					return this.connector.removeAction(t), this;
				},
				unbind: function (t) {
				...
				ready: function () {
					return this.onDomReady(this.initFrame), this;
				},
				initFrame: function () {
				...
				addCallback: function (t) {
					return (
						...
						this.client_callback ||
							(this.unbind('callback'), (this.client_callback = !0)),
						...
						e.scope(function () {
							this.setData(t),
								...
								this.action('response', function (t) {
									i.send('cancel'), i.publish('callback', t), n.hide();
								}),
								this.submit();
						});
				},
				submit3ds: function (t, i) {
					return $ipsp.get('acssubmit').scope(function () {
						this.setData(t), this.setWrapper(i);
					});
				},
			});
			...
			e = Class.extend({
				...
				setHidden: function (t) {
					return (
						t &&
							!isBoolean(t) &&
							console.error('field property `hidden` is not an boolean', this),
						(this.hidden = t),
						this
					);
				},
				...
				setValid: function (t) {
					...
					for (i in t) t.hasOwnProperty(i) && e.push([i, t[i]].join(':'));
					return (this.valid = e.concat(this.valid).join(';')), this;
				},
				setRequired: function (t) {
					(this.valid = ''), t && isBoolean(t) && (this.valid = 'required');
				},
				setReadonly: function (t) {
					return (
						t &&
							!isBoolean(t) &&
							console.error(
								'field property `readonly` is not an boolean',
								this
							),
						(this.readonly = t),
						this
			...
			n = Class.extend({
				...
				validate: function () {
					...
					for (t in e)
						e.hasOwnProperty(t) &&
							null === e[t] &&
							i.push(['data param -', t, 'is incorrect'].join(' '));
					return i;
				},
				...
				getUrl: function () {
					var e;
					return (e = this.validate()).length > 0
						? console.error(e.join('\n'))
						: t
								.concat('://')
								.concat(i)
								.concat('/api/checkout?button=')
								.concat(encodeURIComponent(JSON.stringify(this.data)));
				},
				setMerchantId: function (t) {
					isNumber(t) ||
						console.error(
							'parameter `merchant_id` is required and must be integer'
						),
				...
				setResponseUrl: function (t) {
					return (
						isString(t) ||
							console.error('parameter `url` is required and must be string'),
				...
				addField: function (t) {
					return (
						isObject(t) || console.error('parameter field is required'),
						this.data.fields.push(new e(t)),
						this
					);
				},
				addParam: function (t, i) {
					return (this.data.params[t] = i), this;
				},
				...
				calcStartDate: function (t) {
					return t.period && t.every
						? this.getFuturePeriod(t.period, t.every)
						: null;
				},
				...
				getDefaultStartDate: function (t, i) {
					var e = new Date();
					return e.setMonth(e.getMonth() + 1), this.getDateFormat(e);
				},
				getDefaultEndDate: function () {
					var t = new Date();
					return t.setFullYear(t.getFullYear() + 5), this.getDateFormat(t);
				},
	...
	function () {
		var t = { frameborder: '0', allowtransparency: 'true', scrolling: 'no' },
			i = Module.extend({
				name: '$ipsp.frame3ds',
				frame: null,
				form: null,
				...
				send: function (t, i) {
					return pubsub.publish([this.name, t].join(':'), i || {}), this;
				},
				removeAction: function (t) {
					return pubsub.unsubscribe(t), this;
				},
				...
				initEvents: function () {
					this.addEvent(
						...
						this.proxy(function () {
							this.loadConnector(),
								this.send('load', {}),
								this.addCss(this.iframe, { height: '720px' });
						})
					);
				},
				initFrame: function (i) {
					(this.iframe = this.find(i) || document.createElement('iframe')),
						...
						this.addCss(this.iframe, { width: '100%', height: '100%' });
				},
				initConnector: function () {
					(this.connector = new Connector()),
						this.connector.action(
							'response',
							this.proxy(function (t, i) {
								this.send(i, t);
							})
						);
				},
				...
				setWrapper: function (t) {
					if (
						(isString(t) ? (this.wrapper = this.find(t)) : (this.wrapper = t),
						!t)
					)
				...
				submitHelp: function () {
					(this.form = prepareFormData(
						this.options.url,
						this.options.send_data,
						this.name
					)),
	...
	function () {
		var t = Module.extend({
			...
			setWrapper: function (t) {
				if (!t) throw Error('asc submit wrapper element is undefined');
				return (
					isString(t)
						? (this.wrapper = this.find(t))
						: isElement(t) && (this.wrapper = t),
					this
				);
			},
			submit: function () {
				return (
					...
					this.popup && popupBlocker(this.popup)
						? this.form.submit()
	...
	function () {
		...
		var i = Module.extend({
			init: function () {
				(this.wrapper = this.elem('div', t.wrapper)),
					...
					this.addEvent(
						...
						'click',
						this.proxy(function (t) {
							t.preventDefault(), this.hide(), this.close();
						})
					),
					...
					this.addCss(this.content, { position: 'relative', zIndex: '2' }),
			...
			show: function () {
				this.addCss(this.wrapper, { display: 'block' }),
					this.addCss(this.find('body'), { overflow: 'hidden' }),
					this.addCss(this.find('html'), { overflow: 'hidden' });
			},
			hide: function () {
				this.addCss(this.wrapper, { display: 'none' }),
					this.addCss(this.find('body'), { overflow: '' }),
					this.addCss(this.find('html'), { overflow: '' }),
	...
	function () {
		(this.MobilePay = Module.extend({
			setConnector: function (t) {
				return (this.connector = t), this;
			},
			config: function (t) {
				this.options = t;
			},
			setup: function (t) {
				return (this.payment = t), this.check(), this;
			},
			supported: function () {
				var t;
				try {
					t = CSS.supports('-webkit-appearance: -apple-pay-button');
				} catch (i) {
					t = !1;
				}
				return t;
			...
			initButton: function () {
				(this.container = this.find(this.options.container)),
					(this.button =
						this.find(this.options.button) || document.createElement('button')),
			...
			paymentRequest: function () {
				...
				try {
					(t = new PaymentRequest(
						...
						t.addEventListener(
							'merchantvalidation',
							this.proxy(this.merchantValidation)
						),
						t.addEventListener(
							'paymentmethodchange',
							this.proxy(this.paymentMethodChange)
						),
						t.addEventListener(
							'shippingaddresschange',
							this.proxy(this.shippingAddressChange)
						),
						t.addEventListener(
							'shippingoptionchange',
							this.proxy(this.shippingOptionChange)
						);
				} catch (t) {}
				return t;
			},
			pay: function () {
				...
				this.request
					...
					.catch(function (i) {
						t.publish('log', { event: 'lastError', type: 'ipsp', details: i });
					});
			},
